...
- **Demo GIF in the README and in the packaged extension**: the Marketplace listing and the
  repository README now open with a recording of the real in-editor sequence. An empty
  Markdown link is underlined as a `Z108` error, `Ctrl+.` applies the Quick Fix, and the
  `TODO` placeholder that fix injects is then replaced with real link text until the file
  reports zero findings. The clip deliberately runs past the Quick Fix: the injected `TODO`
  is itself flagged `Z501 PLACEHOLDER_TEXT`, so stopping at the fix would have ended on a
  warning. `images/demo.gif` is 800×450, 12 fps, 14.9 s, ~260 KB; `.vscodeignore` no longer
  excludes `images/demo*.gif`, so the asset ships inside the `.vsix` (against the 5 MB
  packaging ceiling enforced in CI).
  - The asset is regenerated by `docker/record-demo.sh`, which now ends by invoking
    `docker/encode-demo.sh`, so one `docker run` spans record → encode → seed palette →
    verify. `scripts/verify_demo_gif.py` gates the result and the encoder writes to a
    staging name, so a GIF that fails verification is never left on disk.
  - **Two defects in the earlier draft of this asset were found by measuring it and are
    fixed here.** Its editor text rendered at a glyph x-height of 17px against the
    README's own 9px prose — 1.9× — because a 2× device scale factor was applied on a 1×
    virtual screen and `optimize-gif.js`'s `scale='min(width,iw)'` was then a no-op at the
    same width, so nothing downscaled it. And `palettegen` allocated `#C96256` as its
    nearest colour to `editorError.foreground` (`#F14C4C`), a distance of 46.7, so the
    error squiggle — the asset's entire subject — rendered as a pale pink. `optimize-gif.js`
    gained `--seed`, which forces exact colours into the palette, and `--dither` now
    defaults to `none` so a seeded colour is actually reached rather than dithered around.
